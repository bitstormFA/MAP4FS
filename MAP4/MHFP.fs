...
let shinglesFromMol (mol: RWMol) (radius: int) (rings: bool) (minRadius: int) =
...
let shinglesFromMolDefaults mol = shinglesFromMol mol 3 true 1


let shinglesFromSmiles (smiles: Smiles) (radius: int) (rings: bool) (minRadius: int) (sanitize: bool) =
    maybe {
        let! mol = smilesToMol smiles sanitize
        return shinglesFromMol mol radius rings minRadius
    }
...
let secfpFromMol (mol: RWMol, length: int, radius: int, rings: bool, minRadius: int) =
    shinglesFromMol mol radius rings minRadius
    |> hash
    |> fold length
